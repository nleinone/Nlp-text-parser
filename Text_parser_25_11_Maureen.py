...
def co_occ_combiner(lista1):
    ...
    # for plot
    co_word=['']*len(co_occuring_list)
    co_value=['']*len(co_occuring_list)
    for i in range(len(co_occuring_list)):
        co_word[i]=co_occuring_list[i][0]
        co_value[i]=co_occuring_list[i][1]
    plt.bar(co_word, co_value)
    plt.xlabel('Words')
    plt.ylabel('Frequency')
    plt.title('Most co-occuring words')
    plt.show()
